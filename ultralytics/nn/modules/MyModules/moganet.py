...
from .PConv3 import Partial_conv3
...
class MultiOrderDWConv(nn.Module):
    ...
    def __init__(self,
        ...
        #     out_channels=embed_dims,
        #     kernel_size=1)
        self.PW_conv = Partial_conv3(dim=embed_dims, channel_out=embed_dims, n_div=4, forward='split_cat')
...
class MultiOrderGatedAggregation(nn.Module):
    ...
    def __init__(self,
        ...
        # self.gate = nn.Conv2d(
        #     in_channels=embed_dims, out_channels=embed_dims, kernel_size=1)
        self.gate = Partial_conv3(dim=embed_dims, channel_out=embed_dims, n_div=4, forward='split_cat')
        ...
        #     in_channels=embed_dims, out_channels=embed_dims, kernel_size=1)
        self.proj_2 = Partial_conv3(dim=embed_dims, channel_out=embed_dims, n_div=4, forward='split_cat')
...
class MogaBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.inverse = InvertedResidual(dim=embed_dims, mlp_ratio=4., drop=0., drop_path=0.,
    ...
    def forward(self, x):
        ...
        # channel
        identity = x
        x = self.layer_scale_2 * self.inverse(self.norm2(x))
        x = identity + self.drop_path(x)
        return x
...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 1000, 'input_size': (3, 640, 640),
...
@register_model
def moganet_xtiny(pretrained=False, fork_feat=True):
    model = MogaNet(arch='x-tiny', fork_feat=fork_feat)
...
@register_model
def moganet_tiny(pretrained=False, fork_feat=True):
    model = MogaNet(arch='tiny', fork_feat=fork_feat)
...
@register_model
def moganet_tiny_sz256(pretrained=False, fork_feat=True):
    model = MogaNet(arch='tiny', fork_feat=fork_feat)
    model.default_cfg = default_cfgs['moganet_t']
    if pretrained:
...
@register_model
def moganet_small(pretrained=False, fork_feat=True):
    model = MogaNet(arch='small', fork_feat=fork_feat)
...
@register_model
def moganet_base(pretrained=False, fork_feat=True):
    model = MogaNet(arch='base', fork_feat=fork_feat)
...
@register_model
def moganet_large(pretrained=False, fork_feat=True):
    model = MogaNet(arch='large', fork_feat=fork_feat)
...
@register_model
def moganet_xlarge(pretrained=False, fork_feat=True):
    model = MogaNet(arch='x-large', fork_feat=fork_feat)
...
if __name__ == "__main__":
    model = moganet_tiny(pretrained=True,fork_feat=True).to('cuda')
    int = torch.randn(1, 3, 640, 640).cuda()
    out = model(int)
    for i in out:
        print(i.shape)
